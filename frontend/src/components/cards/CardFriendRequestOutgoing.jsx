...
const { useCancelFriendRequest } = apiFriendHooks;

const CardFriendRequestOutgoing = ({ request = {} }) => {
  const { recipient: user = {} } = request;
  ...
  const Actions = () => {
    return (
      <button
        ...
        onClick={() => mutateCancelFriendRequest(request?._id)}
        disabled={isPendingCancelFriendRequest}
      >
        Cancel Request
      </button>
    );
...
export default CardFriendRequestOutgoing;
